...
if (!empty($_POST)) {
    if (isset($_POST['prendreRDV'])) {                  
        priseRdv($_SESSION["id"],$_POST['idRdv']);      ///Si on prend rdv avec le coach selectionne sur la page
    ...
    if (isset($_POST['search'])) {                       ///Si on appuie sur le bouton rechercher dans la navbar
        ...
        if(checkSearch($search)){                       ///On check si la recherche de specialite est valide
            ...
            header("Location: search.php");              ///On va sur une page si c'est valide
        }
        if (checkSearchNomPrenom($search))
        {
            ...
            header("Refresh: autreCoachs.php");         ///Sinon on refresh la page actuelle
        }
...
<body>
    ...
    <div id="tableau">
        <div class="col-md-12 search-table-col">
            <form method="POST" action="">
                <input type="date" id="datefield1" name="dateChoisie" value="min" min="2018-01-01" max="2025-12-31">
                <script>
                    var today = new Date();
                ...
                <button class='btn btn-primary' name='Actualiser' type='submit' style='background: rgb(0,0,0);'>Actualiser</button>
            </form>
                    ...
                    <tbody>
                        ...
                        if (!empty($_POST)) {
                            if (isset($_POST['Actualiser'])) {
                                ...
                                mesCoachs($_SESSION["specialite"],$_POST["dateChoisie"]);
                            }
                        }
                        //mesCoachs($specialite);
                        ?>
                    </tbody>
                </table>
            </div>
    ...
    <footer class="text-white bg-dark" style='margin-top: 40vh'>
